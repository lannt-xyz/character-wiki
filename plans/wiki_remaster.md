...
3. **Cập nhật wiki_characters** từ v2 data (phase cuối — chính xác nhất)
...
[wiki_remaster.py]
  ...
  Phase 2: Backup DB (trước bất kỳ write nào)
  Phase 3: Build character markdown: v1 snapshots + raw chapter excerpts
  Phase 4: LLM extract artifacts per character → wiki_seed.json
  Phase 5: Pre-remaster top 20 chars → UPDATE wiki_characters (enriches Phase 7 context)
  Phase 6: Seed artifacts vào DB → wiki_artifacts + wiki_artifact_snapshots
  Phase 7: Extraction loop — re-read toàn bộ 707 chapter batches → v2 snapshots
  Phase 8: Final remaster — re-synthesize wiki_characters từ v2 snapshots
        ↓
[wiki_batches không bị đụng — main_wiki.py vẫn thấy MERGED hết]
```

**Tại sao thứ tự này?**
- Phase 3 dùng raw chapter text → Phases 4/5 tìm được data bị bỏ sót (không chỉ re-cook v1)
- Phase 5 chạy trước Phase 7 để enrich `character_context` cho extraction loop
- Phase 7 là bước chính — LLM re-read all 707 batch × 5 chapters → v2 snapshots
- Phase 8 chạy SAU Phase 7 → wiki_characters cuối cùng phản ánh v2 data (chính xác nhất)
...
uv run python3 wiki_remaster.py --from-phase 7   # chỉ chạy lại extraction loop + final remaster
